 ...
 * Tools: search_tools, get_tool, list_fresh
 */
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === 'search_tools') {
      ...
      // strip undefined constraint keys
      ...
      return {
        content: [{ type: 'text', text: JSON.stringify(responseEnvelope({ count: results.length, results }), null, 2) }],
      };
    }
    ...
    if (name === 'get_tool') {
      ...
      if (!tool) {
        return { content: [{ type: 'text', text: JSON.stringify({ error: 'not found', id }) }], isError: true };
      }
      return { content: [{ type: 'text', text: JSON.stringify(responseEnvelope({ tool }), null, 2) }] };
    }
    ...
    if (name === 'list_fresh') {
      ...
      return {
        content: [{ type: 'text', text: JSON.stringify(responseEnvelope({ since: since ?? null, count: tools.length, tools }), null, 2) }],
      };
    }
